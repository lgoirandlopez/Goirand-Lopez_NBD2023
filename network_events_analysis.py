...
class NetworkEventsAnalysis():
    def __init__(self,spk,NE=None,timedel=7,dim=None,framerate=None,SliceName='') :
        '''        
        ...
        timedel : Int, optional
            Extent in frames of the studying window around the NE Peak-time. The default is 7.
        dim : List, optional
            List with dimensions of the records, the number of recorded cells and the length of recording frames. 
            The default is None.
        framerate : Float, optional
            The recording framerate in frame/s. The default is None.
        SliceName : TYPE, optional
        ...
        if dim is None :
            self.dim = np.shape(spk)
            self.indcell = np.arange(np.shape(spk)[0])
        else :
            self.dim = dim
            self.indcell = np.arange(dim[0])
    ...
    def detection_network_events(self,spk,Windowslength=4,Iterations=1000,optionsDisplay='Frames',
        ...
        Windowslength : Int, optional
            Frame extent of the slinding window to calculate Network Events. The default is 4.
        Iterations : Int, optional
            Number of iteration for created surrogate dat sets. The default is 1000.
    ...
    def NEwindowfun(self,data,NE,timedel,NEwind='pre',measoption='bin',measwinddim='1D',bothdirectionmeas = False) :
        '''
        ...
        timedel : Int, optional
            The delay (in frames) defining extent of the window. The default is 3.
        ...
        measoption : String, optional
            ...
            If 'delay', the data will be binarise and then non-null value will be the delay to the NE date.
                If NEwind = 'full', the delay is calculatd from the start of window centered in the NE (1 - 2*timedel+1).
        ...
        if measoption.lower() == 'bin' :
            ...
            print('-----------------' + 'If the window option is full, the delay is calculatd from the start of window centered in the NE (1 - 2*timedel+1).' + '-----------------')
            if measwinddim.lower() == '1d' :
        ...
        # --------------------- Preparation of data ---------------------
        if NEwind.lower() == 'pre' :
            workwindow = np.array([np.arange(ne-timedel,ne) for ne in NE])
            workwindownodate = np.array([np.arange(-timedel,0) for ne in NE])
        elif NEwind.lower() == 'post' :
            workwindow = np.array([np.arange(ne+1,ne+timedel+1) for ne in NE])
            workwindownodate = np.array([np.arange(1,timedel+1) for ne in NE])
        elif NEwind.lower() == 'full' :
            workwindow = np.array([np.arange(ne-timedel,ne+timedel+1) for ne in NE])
            if not bothdirectionmeas :
                workwindownodate = np.array([np.arange(1,2*timedel+2) for ne in NE])
            else :
                workwindownodate = 2*timedel+2-np.array([np.arange(1,2*timedel+2) for ne in NE])
    ...
    def thresNEwindshufflefun(self,data,NE,timedel,nsurr=1000,siglevel = .05,NEwind='pre',measoption='bin',measwinddim='1D',bothdirectionmeas = False,**pwdist_kwargs) : 
        '''
        ...
        measoption : String, optional
            ...
            If 'bin', the data will be binarize, 1 if activity and 0 if no activity in the window.
            If 'delay', the data will be binarise and then non-null value will be the delay to the NE date.
                If NEwind = 'full', the delay is calculated from the start of window centered in the NE (1 - 2*timedel+1).
            If 'date', the data will be the date of the activity in the window, 0 if no activity in the window.
            The default is 'bin'.
        measwinddim : String, optional
            Selection of the dimension of the transformed data.
        ...
        **pwdist_kwargs 
        ...
        # --------------------- Preparation of data ---------------------
        NEwindow = np.array([np.arange(ne-timedel,ne+timedel+1) for ne in NE])
        if NEwind.lower() == 'pre' :
            workwindow = np.array([np.arange(ne-timedel,ne) for ne in NE])
            workwindownodate = np.array([np.arange(-timedel,0) for ne in NE])
            selectedindx = np.arange(0,timedel)
        elif NEwind.lower() == 'post' :
            workwindow = np.array([np.arange(ne+1,ne+timedel+1) for ne in NE])
            workwindownodate = np.array([np.arange(1,timedel+1) for ne in NE])
            selectedindx = np.arange(timedel+1,2*timedel+1)
        elif NEwind.lower() == 'full' :
            workwindow = np.array([np.arange(ne-timedel,ne+timedel+1) for ne in NE])
            workwindownodate = np.array([np.arange(1,2*timedel+2) for ne in NE])
            if bothdirectionmeas  :
                workwindownodate2 = 2*timedel+2 - np.array([np.arange(1,2*timedel+2) for ne in NE])
                print('#'*10+'Data are measured in both direction for cosine measure'+'#'*10)
            selectedindx = np.arange(0,2*timedel+1)
...
# ------------------------------ Display function ------------------------------
    ...
    def CHA(self,dataforcluster,metric='cosine',linkagechoice='average',threscalcul=True,thres=None,nsurr=1000,siglevel=.05,**cluster_kwarg) :
        ...
        **cluster_kwarg : 
        ...
        if threscalcul :
            ...
            self.distthresclust = self.thresNEwindshufflefun(self.spk,self.NE,timedel=self.timedel,
        ...
        if len(dataforcluster) > 1 :
            if not self.bothdirectionmeas :
                ...
                dataforcluster2 = self.NEwindowfun(self.spk,self.NE,timedel=self.timedel,NEwind=self.NEwind,measoption=self.measoption,measwinddim=self.measwinddim,bothdirectionmeas=self.bothdirectionmeas)
    ...
    def NEwindowfunClusters(self,NEwind='full',measoption='delay',measwinddim='fullwind',clustermethod='CHA',bothdirectionmeas=False,**cluster_kwarg):
        '''
        ...
        clustermethod : String, optional
            Method of clustering choosen. The default is 'CHA'.
        **cluster_kwarg 
        ...
        self.dataforcluster = self.NEwindowfun(self.spk,self.NE,timedel=self.timedel,NEwind=self.NEwind,measoption=self.measoption,measwinddim=self.measwinddim)
    ...
    def SigCrossCorrelation(self,data1,data2,delay,nsurr = 1000) :
        '''
